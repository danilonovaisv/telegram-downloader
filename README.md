...
3. Set up your environment variables. You need to provide values for the following variables:
   ...
   | `BOT_API_DIR` | The directory where the local bot API stores its files (required only when `TELEGRAM_LOCAL` is `true`). You can alternatively edit the docker-compose file to use a Docker volume instead. In my case, I wanted to store it in a specific directory. |
    ...
    > **Tip:** When `TELEGRAM_LOCAL` is `false` you don't need to run the local Bot API server and the bot will download files directly using Telegram's hosted infrastructure.
